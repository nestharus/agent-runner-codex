...
record the current user/net/mnt/pid identities from /proc/self/ns. The four
...
class InventoryProducerFailureTest(unittest.TestCase):
    ...
    def setUp(self):
        ...
        boundary = json.loads((private/'boundary.json').read_text())
        self.assertTrue(boundary['masks'] and boundary['read_only_inputs'] and boundary['DAC_denied'])
        for name, identity in boundary['namespaces'].items():
            self.assertEqual(os.readlink('/proc/self/ns/'+name), identity)
