...
def require_private_boundary(report_path):
    """Match the caller's probe to this process; never claim this proves isolation."""
    if not report_path.is_absolute() or not report_path.is_file():
        raise ValueError("requires an existing absolute private boundary probe report")
    boundary = json.loads(report_path.read_text())
    for key in ("masks", "read_only_inputs", "DAC_denied"):
        if boundary.get(key) is not True:
            raise ValueError(f"private boundary probe did not establish {key}")
    namespaces = boundary.get("namespaces", {})
    if set(namespaces) != {"user", "net", "mnt", "pid"}:
        raise ValueError("private boundary probe must record user/net/mnt/pid")
    for name, identity in namespaces.items():
        if os.readlink("/proc/self/ns/" + name) != identity:
            raise ValueError(f"private boundary probe identity changed: {name}")
