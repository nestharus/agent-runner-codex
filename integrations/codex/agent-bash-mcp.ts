...
new Function("require", "module", "exports", await bundled.outputs[0].text())(
...
export async function sessionId(timeoutMs = 5000): Promise<string> {
...
export async function callBash(args: unknown, abort: AbortSignal): Promise<string> {
  ...
  for await (const line of input) {
    ...
    switch (message.method) {
      ...
      case "tools/call": {
        ...
        const promise = callBash(message.params.arguments, controller.signal).then(
